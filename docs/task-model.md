...
因此数据模型必须容纳运行时发现的真实阻塞原因，而不仅仅是规划阶段写下的依赖图。
...
4. 若验证不通过，则进入等待，并更新当前的软依赖提示。
5. 若验证通过，则创建 WorkTree 并推进实现、测试、PR。
6. 跟进 checks / review / merge。
7. 清理 WorkTree，刷新本地基线。
8. 以成功或失败的终态结束。

这里有一个关键点：

- 规划层的 `dependencies` 只是“高质量初始猜测”。
- 运行层真正的阻塞原因，是 `Assumptions` 是否成立。
