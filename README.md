...
CZ-Stack 是一份面向新项目启动的 TypeScript monorepo 模板，当前首版基线已经落地 **Web 前端、Hono API、oclif CLI、contract 中心与 SQLite-first db 边界**，并通过统一脚本、共享契约与分层验证入口保持协作一致。
...
- [`modules/db`](modules/db) — `@cz-stack/db`，提供 SQLite-first 的 adapter 边界与默认实现。
- [`tests`](tests) — contract / api / db / cli / web 的统一验证入口。
...
- SQLite-first 的 `db` 边界与默认 adapter，占位未来替换实现。
